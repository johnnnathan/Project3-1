...
A Transformer model is trained on **Event-Human3.6M (EH36M)** to predict 2D body joints from event streams.
2. **Skeleton-based action recognition**
The predicted poses are fed into a pretrained **MotionBERT** classifier (NTU RGB+D 60, x-sub) to obtain a high-level action label (e.g. *sit down*, *walk towards*, *clapping*).
This setup is a step towards **privacy-friendly behaviour analysis** for the EDMO educational robots.
---
## Project Structure
```bash
EH36M/
...
└── labels/
    └── ntu60_labels.py          # NTU RGB+D 60 action names
...
- For EDMO classrooms, the same pipeline can be applied to **real event-camera recordings** of students working with robots, and actions can later be mapped to collaboration indicators (e.g. *walking towards*, *hugging*, *hand waving* → social interaction).
